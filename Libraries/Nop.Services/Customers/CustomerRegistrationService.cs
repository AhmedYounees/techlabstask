...
public partial class CustomerRegistrationService : ICustomerRegistrationService
{
    ...
    public virtual async Task<CustomerRegistrationResult> RegisterCustomerAsync(CustomerRegistrationRequest request)
    {
        ...
        var crmApiEndpoint = "https://localhost:44369/api/customers";

        var crmResponse = await _httpClient.PostAsJsonAsync(crmApiEndpoint, request);

        if (!crmResponse.IsSuccessStatusCode)
        {
            var errorMessage = await crmResponse.Content.ReadAsStringAsync();
